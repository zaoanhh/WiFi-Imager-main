using DrWatson, Revise, Clustering
...
for file_index in axes(material_names, 1)
    for sample_index in 1:sample_num
        temp_file_name = joinpath(datadir("solid_res", "born_nompi"), "$(material_names[file_index])_$(sample_index).jld2")
        if isfile(temp_file_name)
            ...
            for material_index in axes(material_labels[file_index], 1)
                ...
                features_all_now = Vector{Any}()
                for freq_index in 1:length(selected_subcarriers)
                    scatter_now = imag.((temp_data[:, :, freq_index] ./ (-1im * parameters_all[freq_index].k₀ / parameters_all[freq_index].η)) .* rectangle_now)
                    uni_fea = unique(scatter_now)
                   push!(features_all_now,uni_fea[findall(uni_fea .!= 0)])
                end
                R = kmeans(vcat(features_all_now...)', 10; maxiter=200,display=:none)
                push!(features, vec(R.centers))
            end
        end
    end
end
...
X_df = DataFrame(hcat(features...)', :auto)
...
tree = Tree()
evaluate(tree, X_df, y_df, resampling=CV(nfolds=5),
    measures=[accuracy])

Forest = MLJ.@load RandomForestClassifier pkg=DecisionTree
forest = Forest()
evaluate(forest, X_df, y_df, resampling=CV(nfolds=5),
    measures=[accuracy])


SVM = MLJ.@load SVC
svm = SVM()
evaluate(svm, X_df, y_df, resampling=CV(nfolds=5),
    measures=[accuracy])
## use MLP
